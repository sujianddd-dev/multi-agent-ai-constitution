...
> ⚠️ **模板定制提醒**：本文件是通用模板。投入使用前必须完成定制（占位符、Agent 分工、技术栈、目录结构、任务、主 Agent 任命、API 契约对齐）——**未定制前不得视为已生效的宪法**。定制清单见 ai-constitution.skill 的 SKILL.md。
...
| 顺序 | 文档 | 预计时间 | 作用 |
|---|---|---|---|
| 🔥 | [STATE.md](./STATE.md) | 1 分钟 | **接力棒：现在到哪了**（接手第一读，切 Agent/模型必看） |
...
| [STATE.md](./STATE.md) | **接力棒**：当前任务、进度光标、下一步、阻塞、已知坑、验收结果 | **每轮收尾由执行 Agent 更新**（不更新=本轮未完成） |
...
- 在开始工作前，**必须阅读上方全部文档**；**接手已有工作时，第一件事读 [STATE.md](./STATE.md)**
- 遵循 AGENT_GUIDELINES.md 中的规则
- 任何不确定的地方，**保守对待**（不做比做错好）
- **优先提 PR**，由人类或主 Agent 审批，禁止直接合并
- **每轮收尾更新 [STATE.md](./STATE.md)**——不更新视为本轮未完成
- 发现 STATE 与代码不一致时：**以代码为准**，先更新 STATE 再动手

---

## 🔁 交接与验收（Handoff & Verify）

> 切 Agent / 切模型时，**丢的从来不是代码，是进度上下文**。本节用两样东西兜住它。

### 1. 接力棒：[STATE.md](./STATE.md)

固定骨架、≤50 行、只写"现在"：当前任务 / 进度光标 / 下一步 / 阻塞 / 已知坑 / 验收结果。

| 时机 | 动作 |
|---|---|
| **接手时** | 先读 STATE.md，跑一遍它的验收命令，确认与"最近一次"一致，再动手 |
| **每轮收尾** | 更新"更新时间 + 更新者 + 进度光标 + 下一步"——**不更新 = 本轮未完成** |
| **发现不一致** | 以代码为准，先把 STATE 改对，再继续（并在这里写一句为什么） |

### 2. 交接单：PR 模板

所有 PR 按 [`.github/PULL_REQUEST_TEMPLATE.md`](./.github/PULL_REQUEST_TEMPLATE.md) 的"**交接五问**"填写：

1. **依据**：这次改动遵循哪条规范 / 哪条决策
2. **改了什么**：文件级清单 + **明确未做的范围**
3. **怎么验证**：命令 + 结果（无法验证须标注"待人类验证"）
4. **遗留什么**：已知问题 / 边界 / 技术债
5. **下一步给谁**：建议的下一个动作与接手人（**含"STATE.md 是否已更新"**）

### 3. 验收：一条与模型无关的命令

宪法必须写死**一条命令**作为验收锚点（示例：`pytest` / `npm test` / `make verify`），并记录在 STATE.md 的「✅ 验收」一节。

> 为什么重要：换模型最容易发生的是"质量漂移"。**同一条命令、同一个期望输出**，是唯一不随 Agent 和模型改变的标准。
